	...
	for (const file of fs.readdirSync('./translate/from', 'utf-8')) {
		if (file.endsWith('.json') && file.startsWith(project + '.')) {
			...
			for (const element of data) {
				...
				console.clear();
				console.log('[   ] '+file+'('+target+') => ' + Math.floor(perc * 10000)/100 + '% ETA '+Math.floor(remainder/3600)+':'+Math.floor(remainder%3600/60)+':'+Math.floor(remainder%60));
				let tryCount = 0;
				...
				while (typeof translated[element.string] === 'undefined') {
					...
					if (res.status === 200) {
					    ...
					    console.clear();
						console.log('[200] '+file+'('+target+') => ' + Math.floor(perc * 10000)/100 + '% ETA '+Math.floor(remainder/3600)+':'+Math.floor(remainder%3600/60)+':'+Math.floor(remainder%60));
						await wait();
					} else if (res.status === 403) {
						const error = (await res.json()).error;
						if (error === 'Too many request limits violations') {
							console.clear();
							console.log('[403] '+file+'('+target+') => ' + Math.floor(perc * 10000)/100 + '% ETA '+Math.floor(remainder/3600)+':'+Math.floor(remainder%3600/60)+':'+Math.floor(remainder%60));
						...
						console.clear();
						console.log('[429] '+file+'('+target+') => ' + Math.floor(perc * 10000)/100 + '% ETA '+Math.floor(remainder/3600)+':'+Math.floor(remainder%3600/60)+':'+Math.floor(remainder%60));
